...
const pool = mysql.createPool({
  ...
  database: 'isett', // le nom de votre base de données
});
...
app.get('/', (req, res) => {
  res.send('Welcome to the API'); // ou res.sendFile(__dirname + '/index.html') si vous avez une page d'accueil HTML
});
// Get all students
app.get('/etudiant', async (req, res) => {
  try {
    const result = await pool.query('SELECT cin, nom, prenom, numtel, email, department, classe FROM etudiant');
    console.log("fetch successful from table etudiant database isett:", result.rows);
